...
String sendPhotoTelegram() {
  ...
  // **Turn ON the flash before taking the picture**
...
void loop() {
  ...
  if (motionDetected == HIGH && !sendPhoto) {  // If motion detected and no ongoing photo request
    sendPhoto = true;  // Set flag to indicate photo should be sent
    Serial.println("Motion detected! Preparing photo...");

    // Send Telegram message about motion detection
    String motionMessage = "Motion detected!";
    bot.sendMessage(CHAT_ID, motionMessage, "");
  }

  if (sendPhoto) {
    sendPhotoTelegram();  // Send photo when motion is detected
    sendPhoto = false;  // Reset the flag after sending photo
  }

  if (millis() > lastTimeBotRan + botRequestDelay)  {
    int numNewMessages = bot.getUpdates(bot.last_message_received + 1);
    while (numNewMessages) {
      handleNewMessages(numNewMessages); // Handle incoming messages
